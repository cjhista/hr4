...
// Define department colors (sync for chart and legend)
$deptColors = ['#3B82F6','#10B981','#F59E0B','#EF4444','#8B5CF6'];
$deptChartColors = [];
foreach ($deptLabels as $index => $label) {
    $deptChartColors[] = $deptColors[$index % count($deptColors)];
}
...
// JSON for charts
// ---------------------
$deptJson = json_encode(['labels' => $deptLabels, 'data' => $deptValues, 'colors' => $deptChartColors]);
...
<span class="inline-block w-3 h-3 rounded-full" style="background-color: <?= $deptColors[$index % count($deptColors)] ?>;"></span>
...
<div class="w-full h-80"><canvas id="statusChart"></canvas></div>
</div>
</div>
...
<?php else: ?>
<?= (isset($emp['status']) && strtoupper($emp['status']) === 'ACTIVE') ? "<span class='text-green-600 font-semibold'>Active</span>" : "<span class='text-yellow-600 font-semibold'>Inactive</span>" ?>
<?php endif; ?>
</td>
...
<span class="text-xs px-2 py-1 rounded-md font-medium <?= $a['status'] == 'warning' ? 'bg-yellow-200 text-yellow-800' : ($a['status'] == 'success' ? 'bg-green-200 text-green-800' : ($a['status'] == 'danger' ? 'bg-red-200 text-red-800' : 'bg-blue-200 text-blue-800')) ?>">
